...
export class AuthService {
    private readonly getAccessScopes: string = null;
    ...
    constructor(
        ...
        //Set params
        this.getAccessScopes = settings.GetAccessScopes;
        ...
        //Init urls
        this.aadGetCodeUrl_SignIn = 'https://login.microsoftonline.com/' + this.tenantName + `/oauth2/v2.0/authorize?client_id=${this.clientId}&response_type=code&redirect_uri=${encodeURIComponent(this.loginRedirectUri)}&response_mode=query&scope=${this.getAccessScopes}&state=${''}&nonce=${this.nonce}&p=${this.signInPolicy}`;
        this.aadGetCodeUrl_Register = 'https://login.microsoftonline.com/' + this.tenantName + `/oauth2/v2.0/authorize?client_id=${this.clientId}&response_type=code&redirect_uri=${encodeURIComponent(this.registerRedirectUri)}&response_mode=query&scope=${this.getAccessScopes}&state=${''}&nonce=${this.nonce}&p=${this.signUpPolicy}`;
    ...
    getAccessToken(): Observable<string> {
        ...
        if (nowEpoch < (savedStatus.accessTokenExpiresTimeEpoch - this.secureTimeGap)) {
            ...
            return this.httpService.post(`/api/Auth/RefreshTokens?refreshToken=${refreshToken}&policy=${policy}&scopes=${scopes}`, '').pipe(
                map((res: Response) => {
    ...
    initAuthFromCode(code: string, policyType: PolicyType): Promise<boolean> {
        ...
        return this.httpService.post(`/api/Auth/GetTokens?code=${code}&policy=${policyUsed}&scopes=${scopes}`, '').toPromise().then(
            (res: Response) => {
                const wrappedData = new TokensInfoWrapper(<TokensInfo>res.json());
                this.setStoredAuthInfo(
                    policyUsed,
